import React from "react";
import Image from "next/image";

interface NewHomePageCityGalleryProps {
  cityImages: string[];
  isCityLoading: boolean;
  cityRefreshKey: number;
  generateRandomCityImages: () => void;
  registerSection: (el: HTMLElement | null, index: number) => void;
}

const NewHomePage_CityGallery: React.FC<NewHomePageCityGalleryProps> = ({ cityImages, isCityLoading, cityRefreshKey, generateRandomCityImages, registerSection }) => {
  return (
    <section
      id="city-gallery-section"
      className="h-screen w-full py-4 px-3 flex items-center justify-center"
      ref={(el) => registerSection(el, 3)}
    >
      <div className="bg-white/60 backdrop-blur-sm rounded-lg shadow-medium w-full h-[calc(100%-9px)] overflow-y-auto">
        <div className="h-full p-6">
          <h2 className="text-3xl font-bold text-center pt-10 mb-1 font-serif-sc">
            城市速览
          </h2>
          <div className="flex justify-between items-center mb-8">
            <div className="hidden md:block"></div> {/* Empty div for spacing */}

            {/* Desktop buttons - hidden on mobile */}
            <div className="hidden md:flex rounded-full overflow-hidden border border-jade-green shadow-sm">
              <a
                href="/city"
                className="bg-jade-green text-white px-3 py-1 text-xs font-bold hover:bg-jade-green/90 transition-colors duration-300"
              >
                浏览所有
              </a>
              <button
                onClick={generateRandomCityImages}
                className="bg-white text-jade-green px-3 py-1 text-xs font-bold hover:bg-[#f0f9f4] hover:text-jade-green transition-all duration-300 border-l border-jade-green relative cursor-pointer"
                disabled={isCityLoading}
              >
                {isCityLoading ? (
                  <>
                    <span className="opacity-0">换一批</span>
                    <span className="absolute inset-0 flex items-center justify-center">
                      <svg className="animate-spin h-4 w-4 text-jade-green" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                      </svg>
                    </span>
                  </>
                ) : (
                  "换一批"
                )}
              </button>
            </div>
          </div>

          {/* Mobile buttons - shown only on mobile */}
          <div className="flex md:hidden justify-center mt-4 mb-8 rounded-full overflow-hidden border border-jade-green shadow-sm mx-auto w-fit">
            <a
              href="/city"
              className="bg-jade-green text-white px-3 py-1 text-xs font-bold hover:bg-jade-green/90 transition-colors duration-300"
            >
              浏览所有
            </a>
            <button
              onClick={generateRandomCityImages}
              className="bg-white text-jade-green px-3 py-1 text-xs font-bold hover:bg-[#f0f9f4] hover:text-jade-green transition-all duration-300 border-l border-jade-green relative cursor-pointer"
              disabled={isCityLoading}
            >
              {isCityLoading ? (
                <>
                  <span className="opacity-0">换一批</span>
                  <span className="absolute inset-0 flex items-center justify-center">
                    <svg className="animate-spin h-4 w-4 text-jade-green" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                  </span>
                </>
              ) : (
                "换一批"
              )}
            </button>
          </div>

          <div key={cityRefreshKey} className={`grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 transition-all duration-300 ${isCityLoading ? 'blur-effect' : ''}`}>
            {/* Display city images from the 3D folder */}
            {cityImages.map((imagePath, index) => {
              // Extract city name from the image path
              const cityName = imagePath.split('/').pop()?.split('.')[0] || `城市 ${index + 1}`;

              return (
                <div
                  key={`city-${index}-${cityRefreshKey}`}
                  className="bg-white p-1 rounded shadow"
                >
                  <div className="relative aspect-square overflow-hidden">
                    {isCityLoading ? (
                      <div className="absolute inset-0 bg-gray-200 animate-pulse"></div>
                    ) : (
                      <Image
                        src={imagePath}
                        alt={`${cityName} 城市图片`}
                        fill
                        sizes="(max-width: 640px) 33vw, (max-width: 768px) 25vw, 20vw"
                        className="object-cover"
                      />
                    )}
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      </div>
    </section>
  );
};

export default NewHomePage_CityGallery;
